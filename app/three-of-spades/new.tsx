...
export default function NewTOSSession() {
  const router = useRouter();
  const createSession = useTOSStore((s) => s.createSession);
  ...
  const twoDecks = filledPlayers.length >= 6;
  ...
  const startSession = () => {
    ...
    const id = createSession(filledPlayers, firstDealerIndex);
          ...
          <ScrollView style={styles.scroll} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
            ...
            {showSettings && (
              <View style={styles.settings}>
                ...
                {twoDecks && (
                  <View style={styles.twoDecksHint}>
                    <Text style={styles.twoDecksText}>♠♠  Two decks will be used ({filledPlayers.length} players)</Text>
                  </View>
                )}
...
const styles = StyleSheet.create({
  ...
  settingsHeading: { fontSize: 16, fontWeight: '700', color: colors.textPrimary, marginBottom: spacing.md },
  twoDecksHint: {
  ...
  twoDecksText: { fontSize: 13, color: colors.primaryLight, fontWeight: '600' },
  settingsLabel: {
