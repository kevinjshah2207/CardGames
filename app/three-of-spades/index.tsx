...
import { getTOSLeaderboard } from '@/lib/three-of-spades-scoring';

export default function TOSHub() {
  const router = useRouter();
  const sessions = useTOSStore((s) => s.sessions);
  ...
  return (
    <>
      <Stack.Screen options={{ title: '3 of Spades' }} />
      <SafeAreaView style={styles.safe} edges={['bottom']}>
        <ScrollView style={styles.scroll} contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
          <TouchableOpacity style={styles.newBtn} onPress={() => router.push('/three-of-spades/new')} activeOpacity={0.8}>
            <Text style={styles.newBtnIcon}>＋</Text>
            <Text style={styles.newBtnLabel}>New Session</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.rulesLink} onPress={() => router.push('/three-of-spades/rules')} activeOpacity={0.7}>
          ...
          {activeSessions.length > 0 && (
            <View style={styles.section}>
              ...
              {activeSessions.map((session) => {
                ...
                const leader = getTOSLeaderboard(session.players, session.games)[0];
                return (
                  <TouchableOpacity
                    key={session.id}
                    style={styles.sessionCard}
                    onPress={() => router.push(`/three-of-spades/${session.id}`)}
                    activeOpacity={0.8}
                  >
                    <View style={styles.sessionCardLeft}>
                      ...
                      <Text style={styles.sessionCardMeta}>
                        {hasPending
                          ? `Game ${gamesDone + 1} in progress`
                          : `${gamesDone} game${gamesDone !== 1 ? 's' : ''} played`}
                        {leader && gamesDone > 0 ? `  ·  ${leader.name} leading` : ''}
                      </Text>
                    </View>
                    <View style={styles.activePill}>
                      <Text style={styles.activePillText}>{hasPending ? 'Playing' : 'Active'}</Text>
                    </View>
                  </TouchableOpacity>
                );
              })}
          ...
          {completedSessions.length > 0 && (
            <View style={styles.section}>
              ...
              {completedSessions.slice(0, 3).map((session) => {
                const winner = getTOSLeaderboard(session.players, session.games)[0];
                return (
                  <TouchableOpacity
                    key={session.id}
                    style={styles.sessionCard}
...
const styles = StyleSheet.create({
  ...
  emptyIcon: { fontSize: 48, marginBottom: spacing.md, color: colors.textMuted },
